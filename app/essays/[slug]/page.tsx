...
export async function generateStaticParams() {
  return essays.map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const piece = essays.find((p) => p.slug === slug);
  return { title: piece?.title ?? "Not found" };
}

export default async function EssayPiece({ params }: Props) {
  const { slug } = await params;
  const piece = essays.find((p) => p.slug === slug);
  if (!piece) notFound();
    ...
    <div className="max-w-2xl mx-auto px-6 py-20">
      <div className="mb-16">
        <div className="flex items-center gap-3 mb-6 text-xs text-stone-400 uppercase tracking-wider">
          <Link href="/essays" className="hover:text-stone-600 transition-colors">
            Essays
          </Link>
          <span>·</span>
